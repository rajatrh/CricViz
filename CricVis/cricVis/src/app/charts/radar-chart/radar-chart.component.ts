...
import { RadarData } from 'src/app/shared/model/radar-data';
...
export class RadarChartComponent implements OnInit {
  @ViewChild('radarChartDiv', { static: true }) myDiv: ElementRef;
  cardTitle = 'Performance Comparison'
  view = 1

  data = [
    {
      className: 'Overall Career',
      axes: [
      ]
    },
    {
  ...
  localRadarData: RadarData = new RadarData()

  /*
  Reference to the code below (for radar chart) is at http://bl.ocks.org/chrisrzhou/2421ac6541b68c1680f8#index.html
  Futhermore, a few corrections had to be made to get it working.
  */
  RadarChart = {
        ...
        data.forEach(function (group) {
          group.axes.forEach(function (d, i) {
            d.coordinates = { // [x, y] coordinates
              x: config.w / 2 * (1 - (Math.max(d.value, 0) / config.maxValue) * Math.sin(i * config.radians / vis.totalAxes)),
              y: config.h / 2 * (1 - (Math.max(d.value, 0) / config.maxValue) * Math.cos(i * config.radians / vis.totalAxes))
            };
          });
        });
      }
